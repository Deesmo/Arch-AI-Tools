...
async function callArchTool(toolName, args) {
    const url = `${ARCH_TOOLS_BASE_URL}/v1/tools/${toolName}`;
    const headers = { "Content-Type": "application/json" };
    if (ARCH_TOOLS_API_KEY)
        headers["X-API-Key"] = ARCH_TOOLS_API_KEY;
...
const server = new Server({ name: "arch-tools", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        const result = await callArchTool(name, args);
