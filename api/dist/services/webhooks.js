...
async function deliverWebhook(webhookId, url, secret, payload, deliveryId) {
...
export async function fireWebhookEvent(event, agentId, data) {
    try {
...
export async function fireGlobalWebhookEvent(event, data) {
    if (!data.agent_id || typeof data.agent_id !== "string")
        return;
    await fireWebhookEvent(event, data.agent_id, data);
}
