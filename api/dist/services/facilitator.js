...
function getPublicClient(networkId) {
...
export async function checkNonce(nonce, providerId) {
    if (!redis) {
...
export async function releaseNonce(nonce, providerId) {
    if (!redis)
        return;
    await redis.del(`facilitator:nonce:${providerId}:${nonce}`).catch(() => { });
}
// ─── Payment Decoding ─────────────────────────────────────────────────────────
export function decodePayment(paymentB64) {
    try {
        const decoded = Buffer.from(paymentB64, "base64").toString("utf-8");
        const parsed = JSON.parse(decoded);
        if (!parsed.scheme || !parsed.network || !parsed.payload)
            return null;
...
// ─── Verification ─────────────────────────────────────────────────────────────
export async function verifyPayment(paymentB64, paymentDetails, providerId) {
    // 1. Decode
    ...
    if (client) {
        const tokenAddress = (USDC_CONTRACTS[payment.network] || paymentDetails.asset);
        ...
        if (isToken) {
            try {
                ...
                if (BigInt(balance) < paymentAmount) {
                    await releaseNonce(auth.nonce, providerId);
                    return { isValid: false, invalidReason: "insufficient_balance" };
                }
            }
            catch (err) {
                console.warn(`[facilitator] Balance check warning:`, err.message?.slice(0, 100));
            }
            try {
    ...
    try {
        ...
        if (client) {
            try {
                tokenName = await client.readContract({ address: tokenAddress, abi: EIP3009_ABI, functionName: "name" });
                tokenVersion = await client.readContract({ address: tokenAddress, abi: EIP3009_ABI, functionName: "version" });
            }
            catch { /* use defaults */ }
        }
        ...
        console.warn(`[facilitator] Sig verification warning:`, err.message?.slice(0, 200));
        // Non-fatal during beta
    }
    return { isValid: true };
}
// ─── Settlement ───────────────────────────────────────────────────────────────
export async function settlePayment(paymentB64, paymentDetails) {
    const payment = decodePayment(paymentB64);
    if (!payment) {
...
export function getDefaultFeePercent() {
    return parseFloat(process.env.FACILITATOR_FEE_PERCENT ?? "2.5");
}
export function calculateFee(amountAtomic, feePercent) {
...
export function calculateProviderPayout(amountAtomic, feePercent) {
    const amount = BigInt(amountAtomic);
    const feeBps = Math.round(feePercent * 100);
    const fee = (amount * BigInt(feeBps)) / BigInt(10000);
