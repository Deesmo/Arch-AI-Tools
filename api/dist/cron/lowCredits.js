...
export async function runLowCreditsCron() {
    try {
        ...
        logger.info({ lowCredit: lowCreditAgents.length, depleted: depletedAgents.length }, "Low credits batch cron");
        // Send 80% consumption alerts
        await Promise.allSettled(lowCreditAgents.map((a) => a.email
            ? sendEmail80PctAlert(a.email, a.credits, a.id)
            : Promise.resolve()));
        // Send depletion alerts
        await Promise.allSettled(depletedAgents.map((a) => a.email
...
//# sourceMappingURL=lowCredits.js.map