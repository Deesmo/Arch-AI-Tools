...
export const SIGNUP_FREE_CREDITS = parseInt(process.env.SIGNUP_FREE_CREDITS ?? "100", 10);
 ...
 */
export async function claimSignupIdentity(email) {
 ...
 */
export async function enforceSignupLimits(email, ip) {
 ...
 */
export async function issueEmailVerification(agentId, email, creditsToGate) {
...
export async function verifyEmailToken(token) {
    ...
    if (!agent)
        return null;
    if (agent.verifyTokenExpiry && agent.verifyTokenExpiry < new Date())
        return null;
