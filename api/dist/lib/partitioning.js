...
export async function listApiRequestLogPartitions() {
    const rows = await prisma.$queryRawUnsafe(`
    SELECT
...
export async function ensureMonthlyPartitions(monthsAhead = 2) {
    ...
    if (!enabled)
        return { enabled: false, created: 0, monthsAhead };
    ...
    for (let i = 0; i <= monthsAhead; i++)
        targets.push(addMonthsUTC(base, i));
    let created = 0;
...
export async function dropOldApiRequestLogPartitions(cutoff) {
    const enabled = String(process.env.ENABLE_PARTITIONING || "").toLowerCase() === "true";
    if (!enabled)
        return { enabled: false, dropped: 0 };
    const partitioned = await isApiRequestLogPartitioned();
    if (!partitioned)
        return { enabled: true, partitioned: false, dropped: 0 };
    ...
    for (const p of toDrop) {
        ...
        if (!/^ApiRequestLog_\d{4}_\d{2}$/.test(p.name))
            continue;
