...
const CONSENT_PAGE = (clientName, scope, clientId, redirectUri, state, codeChallenge, codeChallengeMethod, error) => {
...
router.get("/authorize", async (req, res) => {
    const { client_id, redirect_uri, response_type, scope = "tools:read tools:execute", state = "", code_challenge = "", code_challenge_method = "", } = req.query;
...
router.post("/authorize", async (req, res) => {
    const { client_id, redirect_uri, scope, state, email, apiKey, code_challenge = "", code_challenge_method = "", } = req.body;
    const client = await prisma.oAuthClient.findUnique({ where: { clientId: client_id } }).catch(() => null);
...
router.post("/token", async (req, res) => {
    ...
    if (grant_type === "authorization_code") {
        ...
        if (authCode.codeChallenge) {
            ...
            if (!code_verifier) {
                res.status(400).json({ error: "invalid_grant", error_description: "code_verifier is required when PKCE was used" });
                return;
            }
            ...
            res.status(400).json({ error: "invalid_grant", error_description: "PKCE is required for public clients" });
            return;
        }
        // Mark code as used
...
router.post("/register", async (req, res) => {
    const { client_name, redirect_uris, grant_types, token_endpoint_auth_method, response_types, } = req.body;
