 ...
 */
toolsRouter.get("/v1/tools/me", requireApiKey, async (req, res) => {
...
toolsRouter.post("/v1/tools/search", async (req, res) => {
    ...
    if (!task)
        return res.status(400).json({ ok: false, error: "missing_task" });
    ...
    const scored = tools
        ...
        if (/(search|scrape|extract|browser|workflow|pdf|ocr)/.test(q) && /(web|files|ai)/.test(t.category || ""))
            score += 1;
        return { t, score };
    })
        .filter((x) => x.score > 0)
