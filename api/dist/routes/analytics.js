...
router.get("/overview", requireAdmin, async (_req, res) => {
    try {
        ...
        const [totalRequests, requestsToday, requestsWeek, requestsMonth, totalAgents, activeAgentsWeek, totalX402Payments, x402RevenueResult, stripeRevenueResult, topTools, topCallers,] = await Promise.all([
...
router.get("/tools", requireAdmin, async (_req, res) => {
    try {
    ...
    try {
        ...
        res.json({
            ...
            total_revenue: {
                ...
                combined_usd_estimate: Math.round((x402Payments.reduce((s, p) => s + parseFloat(p.amountUsdc || "0"), 0) +
                    stripePurchases.reduce((s, p) => s + p.amountCents, 0) / 100 +
                    facilitatorFeeTotal +
                    directoryListingRevenue) * 100) / 100,
            },
        ...
        for (const m of metrics) {
            ...
            if (m.toolName) {
                if (!byTool[m.toolName])
                    byTool[m.toolName] = { count: 0, totalMs: 0, errors: 0 };
                byTool[m.toolName].count++;
                byTool[m.toolName].totalMs += m.responseMs;
                if (m.statusCode >= 400)
                    byTool[m.toolName].errors++;
            }
        }
