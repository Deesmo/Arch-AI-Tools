...
router.post("/ai-oracle", ...toolMiddleware("ai-oracle"), async (req, res) => {
    ...
    const { question, context: oracleContext, reasoning_depth = "standard" } = req.body;
...
router.post("/crypto-market-cap", ...toolMiddleware("crypto-market-cap"), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (ccmpResp.ok) {
                const ccmpJson = await ccmpResp.json();
                const coins = (ccmpJson.Data || []).map((c, i) => ({
                ...
                res.json({ ok: true, currency: "usd", coins, source: "cryptocompare_fallback", data_source: "CoinGecko", attribution: "Powered by CoinGecko API — https://www.coingecko.com", data_license: "CoinGecko Terms apply — https://www.coingecko.com/en/api_terms", request_id: reqId() });
                return;
            }
        }
        catch { /* fall through */ }
...
router.post("/crypto-news", ...toolMiddleware("crypto-news"), async (req, res) => {
    ...
    const parseRss = async (feedUrl, sourceName) => {
        ...
        for (const item of itemMatches.slice(0, n)) {
            ...
            if (title && link)
                items.push({ title: title.replace(/<[^>]+>/g, "").trim(), url: link.trim(), published_at: pubDate ? new Date(pubDate).toISOString() : new Date().toISOString(), source: sourceName });
        }
        return items;
    };
    try {
...
router.post("/domain-check", ...toolMiddleware("domain-check"), async (req, res) => {
    ...
    try {
        ...
        const data = await (r.ok ? r : (await fetch(`https://rdap.org/domain/${clean}`))).json();
...
router.post("/generate-image", ...toolMiddleware("design-create"), async (req, res) => {
...
router.post("/session-create", ...toolMiddleware("session-create"), async (req, res) => {
    const paid = isX402Paid(req);
    if (!paid) {
...
router.post("/session-message", ...toolMiddleware("session-message"), async (req, res) => {
    const paid = isX402Paid(req);
    if (!paid) {
