...
router.get("/text-transform", (_req, res) => {
    res.type("text/html").send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
router.post("/qr-code-free", async (req, res) => {
    const { text, format = "png" } = req.body;
    if (!text) {
        res.status(400).json({ ok: false, message: "text is required" });
        return;
    }
    if (text.length > 500) {
        res.status(400).json({ ok: false, message: "Text too long for free tier (max 500 chars)" });
        return;
    }
    try {
...
router.post("/hash-free", (req, res) => {
    ...
    if (text.length > 10000) {
        res.status(400).json({ ok: false, message: "Text too long for free tier (max 10000 chars)" });
        return;
    }
...
router.post("/transform-free", (req, res) => {
    ...
    if (!text || !mode) {
        res.status(400).json({ ok: false, message: "text and mode required" });
        return;
    }
    if (text.length > 5000) {
        res.status(400).json({ ok: false, message: "Text too long for free tier" });
        return;
    }
