...
async function handleInvoke(req, res, next, toolNameParam) {
    const toolName = String(toolNameParam || "").toLowerCase();
    const tool = await prisma.tool.findUnique({ where: { name: toolName } });
    if (!tool)
        return fail(req, res, 404, "tool_not_found");
    if (!tool.active)
        return fail(req, res, 410, "tool_disabled");
    ...
    if (toolName === "web-scrape" || toolName === "extract-page" || toolName === "browser-task") {
        ...
        const allow = (req.allowedScrapeDomains || "").split(",").map((s) => s.trim().toLowerCase()).filter(Boolean);
        if (allow.length) {
            ...
            try {
                ...
                if (!allowed)
                    return fail(req, res, 403, "scrape_domain_not_allowed", "Domain not allowed for this API key");
            }
            catch {
    ...
    if (tool.schemaJson && typeof tool.schemaJson === "object") {
        try {
            ...
            if (!validate) {
                validate = ajv.compile(tool.schemaJson);
                if (validatorCache.size > 250)
...
invokeRouter.get("/tools/qr-code", async (req, res) => {
    ...
    if (!data)
        return res.status(400).json({ ok: false, error: "missing_data", hint: "Pass ?data=your-text" });
...
invokeRouter.get("/tools/hash", async (req, res) => {
    ...
    if (!input)
        return res.status(400).json({ ok: false, error: "missing_input", hint: "Pass ?input=your-text" });
...
invokeRouter.get("/tools/text-transform", async (req, res) => {
    ...
    if (!text)
        return res.status(400).json({ ok: false, error: "missing_text", hint: "Pass ?text=your-text" });
    const result = await builtin.transformText({ text, mode: String(req.query.mode || "uppercase") });
    return res.json({ ok: true, result });
});
invokeRouter.post("/v1/tools/:toolName", requireApiKey, async (req, res, next) => {
    return handleInvoke(req, res, next, req.params.toolName);
});
// Legacy route
invokeRouter.post("/api/tools/:toolName", requireApiKey, async (req, res) => {
...
async function dispatchTool(toolName, endpoint, body) {
    switch (toolName) {
        // ── Core 8 ──
...
//# sourceMappingURL=toolInvoke.js.map