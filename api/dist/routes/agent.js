...
router.post("/register", async (req, res) => {
    const { name, email: rawEmail, plan, password } = req.body;
    ...
    try {
        ...
        try {
            ...
            const rows = await prisma.$queryRaw `SELECT id FROM "Agent" WHERE email=${email} LIMIT 1`;
            if (rows.length > 0)
                existing = rows[0];
        }
        if (existing) {
...
router.post("/keys/rotate", requireAuth, async (req, res) => {
    const agent = req.agent;
    if (!agent) {
        res.status(401).json({ ok: false, error: "unauthorized", request_id: reqId() });
        return;
...
router.delete("/keys/:prefix", requireAuth, async (req, res) => {
    const agent = req.agent;
