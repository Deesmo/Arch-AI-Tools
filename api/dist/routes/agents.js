...
router.get("/leaderboard", async (req, res) => {
    try {
        const sortBy = (Array.isArray(req.query.sort) ? req.query.sort[0] : req.query.sort) ?? "reputation";
        const limit = Math.min(parseInt((Array.isArray(req.query.limit) ? req.query.limit[0] : req.query.limit)) || 25, 100);
...
router.post("/register", async (req, res) => {
    try {
        const { name, email: rawEmail, description, wallet_address, callback_url, is_public, } = req.body;
...
router.put("/profile", requireAuth, async (req, res) => {
    ...
    try {
        const { name, description, wallet_address, callback_url, is_public, } = req.body;
