 ...
 * GET /v1/admin/whoami
 */
adminRouter.get("/v1/admin/whoami", (req, res) => {
    const ctx = req.admin;
 ...
 *  - since (ISO date)
 */
adminRouter.get("/v1/admin/request-logs", requireScope("logs:read"), async (req, res) => {
 ...
 */
adminRouter.get("/v1/admin/stats", requireScope("system:read"), async (req, res) => {
    ...
    res.json({
        ...
        status_counts: byStatus.map((x) => ({ status: x.status ?? null, count: x._count })),
        top_tools: topTools.map((x) => ({ tool: x.toolName ?? null, count: x._count })),
 ...
 */
adminRouter.get("/v1/admin/billing-report", requireScope("billing:read"), async (req, res) => {
    const days = Math.min(Math.max(Number(req.query.days) || 7, 1), 90);
    const groupBy = String(req.query.groupBy || "agent");
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
    const byField = groupBy === "apikey" || groupBy === "apiKey" ? "apiKeyId" :
 ...
 */
adminRouter.get("/v1/admin/billing-report.csv", requireScope("billing:read"), async (req, res) => {
    const days = Math.min(Math.max(Number(req.query.days) || 7, 1), 90);
    const groupBy = String(req.query.groupBy || "agent");
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
    const byField = groupBy === "apikey" || groupBy === "apiKey" ? "apiKeyId" :
 ...
 */
adminRouter.get("/v1/admin/fraud-signals", requireScope("fraud:read"), async (req, res) => {
    const hours = Math.min(Math.max(Number(req.query.hours) || 24, 1), 720);
    const since = new Date(Date.now() - hours * 60 * 60 * 1000);
    // Top failing tools
    ...
    const errMap = new Map();
    for (const e of errorsByAgent)
        errMap.set(String(e.agentId), e._count);
    ...
    res.json({
        ...
        failing_tools: failingTools.map((x) => ({ tool: x.toolName ?? "(unknown)", error_count: x._count })),
        noisy_ips: noisyIps.map((x) => ({ ip: x.ip, error_count: x._count })),
        offenders_429: offenders429.map((x) => ({ agentId: x.agentId, count: x._count })),
        offenders_auth: offendersAuth.map((x) => ({ apiKeyPrefix: x.apiKeyPrefix, count: x._count })),
 ...
 */
adminRouter.post("/v1/admin/rollup/run", requireScope("ops:write"), async (req, res) => {
 ...
 */
adminRouter.get("/v1/admin/partitions/status", requireScope("system:read"), async (_req, res) => {
 ...
 */
adminRouter.get("/v1/admin/system/status", requireScope("system:read"), async (_req, res) => {
    const enabled = String(process.env.ENABLE_PARTITIONING || "").toLowerCase() === "true";
    const partitioned = await isApiRequestLogPartitioned();
 ...
 */
adminRouter.post("/v1/admin/partitions/ensure", requireScope("ops:write"), async (req, res) => {
    ...
    try {
        ...
        if (prev === undefined)
            delete process.env.PARTITION_PRECREATE_MONTHS;
        else
            process.env.PARTITION_PRECREATE_MONTHS = prev;
 ...
 */
adminRouter.get("/v1/admin/keys", requireScope("admin:read"), async (req, res) => {
...
adminRouter.post("/v1/admin/keys", requireScope("admin:write"), async (req, res) => {
...
adminRouter.post("/v1/admin/keys/:id/revoke", requireScope("admin:write"), async (req, res) => {
