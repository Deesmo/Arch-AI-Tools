...
router.post("/checkout", requireAuthOrSession, async (req, res) => {
    ...
    if (!stripe) {
        res.status(503).json({ ok: false, error: "not_configured", message: "Stripe not configured", request_id: reqId() });
        return;
    }
    const { pack } = req.body;
...
router.post("/subscribe", requireAuthOrSession, async (req, res) => {
    const agent = req.agent;
    if (!agent) {
        res.status(401).json({ ok: false, error: "unauthorized", request_id: reqId() });
        return;
        ...
        try {
            ...
            if (session.mode === "payment") {
                ...
                try {
                    ...
                    sendAdminAlert(`🔁 New Arch Tools subscription — $${amountUsd2}/mo`, `New subscription started!\n\nCustomer: ${agentRecord2?.email ?? "unknown"}\nPlan: ${planLabel}\nCredits/month: ${creditsPerMonth.toLocaleString()}\nAmount: $${amountUsd2}\nStripe session: ${stripeId}`).catch(() => { });
                }
                catch { /* non-fatal */ }
            }
        }
