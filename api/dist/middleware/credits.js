 ...
 */
export function requireCredits(toolName, credits) {
    return async (req, res, next) => {
        const agentId = req.agentId;
        if (!agentId)
            return fail(req, res, 401, "unauthorized", "Missing agent");
        ...
        req.__arch_tool = { toolName, credits };
        return next();
    };
}
export async function debitCredits(agentId, toolName, credits, requestId, meta) {
 ...
 */
export async function reverseCredits(agentId, credits, reference, meta) {
...
export async function getCreditBalance(agentId) {
