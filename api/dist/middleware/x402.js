 ...
 */
function extractPaymentNetwork(paymentHeader) {
    try {
...
async function verifyPayment(paymentHeader, toolName, paymentRequirements) {
    ...
    try {
        ...
        const isSolana = paymentRequirements.network?.includes("solana");
        let finalPaymentReqs;
        let finalPayload;
        ...
        console.log(`[x402] Verify → ${facilitatorUrl}/verify (tool: ${toolName}, format: ${payloadFormat}, network: ${paymentRequirements.network})`);
...
async function settlePayment(paymentHeader, toolName, paymentRequirements) {
    ...
    try {
        ...
        // Detect Solana vs EVM
        const isSolanaSettle = paymentRequirements.network?.includes("solana");
        let finalPaymentReqsSettle;
        let finalPayloadSettle;
...
export function x402Middleware(toolName) {
    return async (req, res, next) => {
        ...
        const price = X402_PRICES[toolName] ?? "0.010";
        const fullPaymentDetails = buildPaymentRequired(toolName, price);
        ...
        let paymentRequirements = fullPaymentDetails.accepts[0]; // fallback
        if (paymentNetwork || paymentAsset) {
            const match = fullPaymentDetails.accepts.find((a) => {
            ...
            if (match) {
                ...
                console.log(`[x402] Matched payment to: ${match.network} / ${match.asset}`);
            }
            else {
