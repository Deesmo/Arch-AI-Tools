...
const authFailsByIp = new Map();
const billedCallsByAgent = new Map();
const MAX_TRACKED_KEYS = 5000;
function pushWindowed(map, key, windowMs) {
    const now = Date.now();
    let arr = map.get(key) ?? [];
    arr = arr.filter((t) => t > now - windowMs);
    arr.push(now);
    if (map.size > MAX_TRACKED_KEYS && !map.has(key))
        map.clear(); // hard memory cap
    map.set(key, arr);
    return arr.length;
}
function securityAnomalyCheck(req, statusCode, toolName, agentId, isX402, isAdmin) {
    try {
