...
export function initOfficialX402() {
    ...
    _middleware = paymentMiddleware(routes, server, undefined, // paywallConfig
    ...
    if (evmWallet)
        console.log(`[x402-official]    EVM wallet: ${evmWallet.slice(0, 6)}...${evmWallet.slice(-4)}`);
    if (solWallet)
        console.log(`[x402-official]    Solana wallet: ${solWallet.slice(0, 4)}...${solWallet.slice(-4)}`);
}
/**
 ...
 */
export function officialX402Middleware(req, res, next) {
    if (!_middleware) {
        next();
        return;
    }
...
//# sourceMappingURL=x402-official.js.map