...
const ALLOWED_AI_MODELS = (process.env.AI_ALLOWED_MODELS || "claude-sonnet-4-6,claude-opus-4-6,claude-haiku-4-5-20251001").split(",").map((s) => s.trim()).filter(Boolean);
// ─── SSRF protection ───
...
async function readBodyTextWithLimit(resp, limitBytes) {
    const chunks = [];
    let total = 0;
    let truncated = false;
...
async function convertFormat(payload) {
    ...
    if (!from || !to)
        return { ok: false, error: "missing_from_or_to", supported };
    try {
        let parsed;
        if (from === "json") {
...
async function transformText(payload) {
    ...
    const modes = {
        ...
        word_frequency: () => {
            const freq = {};
            words.forEach((w) => { const k = w.toLowerCase().replace(/[^a-z0-9]/g, ""); if (k)
                freq[k] = (freq[k] || 0) + 1; });
...
async function extractMetadata(payload) {
    ...
    if (url && typeof url === "string") {
        if (!isSafeUrl(url))
            return { ok: false, error: "blocked_url", detail: "URL is not allowed" };
        try {
...
async function webScrape(payload) {
    ...
    if (url.length > MAX_SCRAPE_URL_LEN)
    ...
    try {
        ...
        clearTimeout(timeout);
        if (!resp.ok)
            return { ok: false, error: "fetch_failed", status: resp.status };
...
async function claudeJson(systemPrompt, userPrompt, timeoutMs = 20_000) {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey)
        return { ok: false, error: "ai_not_configured", detail: "ANTHROPIC_API_KEY not set" };
    try {
        const controller = new AbortController();
        ...
        if (!resp.ok)
            return { ok: false, error: "ai_api_error", detail: json?.error?.message || String(resp.status) };
...
async function aiGenerate(payload) {
    ...
    if (!apiKey)
        return { ok: false, error: "ai_not_configured", detail: "ANTHROPIC_API_KEY not set" };
    try {
        const messages = [{ role: "user", content: prompt.slice(0, 10_000) }];
        const body = {
            model,
        ...
        if (!resp.ok)
            return { ok: false, error: "ai_api_error", status: resp.status, detail: data?.error?.message || data };
...
async function ocrExtract(payload) {
    ...
    if (!image_url && !image_base64)
        return { ok: false, error: "missing_input", detail: "Provide image_url or image_base64" };
    try {
        let imageSource;
        if (image_base64) {
        ...
        const data = (await resp.json());
        if (!resp.ok)
            return { ok: false, error: "ai_api_error", detail: data?.error?.message };
        const text = (data.content || []).filter((b) => b.type === "text").map((b) => b.text).join("\n");
...
async function ipLookup(payload) {
    ...
    // basic validation
    if (!/^[\d.:a-fA-F]+$/.test(cleaned))
        return { ok: false, error: "invalid_ip_format" };
    try {
        const ctrl = new AbortController();
        ...
        if (data.status === "fail")
            return { ok: false, error: "lookup_failed", detail: data.message || "Unknown IP or private range" };
        return {
            ok: true,
            ip: data.query,
...
async function currencyConvert(payload) {
    ...
    if (!from || !to)
        return { ok: false, error: "missing_currency", detail: "Provide from and to currency codes" };
    ...
    try {
        ...
        if (cached && Date.now() - cached.ts < RATE_CACHE_TTL) {
            ...
            if (data.result !== "success")
                return { ok: false, error: "rate_fetch_failed", detail: data["error-type"] || "API error" };
            rates = data.rates;
            rateCache.set(fromUpper, { rates, ts: Date.now() });
        }
        if (!rates[toUpper])
            return { ok: false, error: "unsupported_currency", detail: `'${toUpper}' not found` };
...
async function timezoneConvert(payload) {
    ...
    try {
        ...
        if (isNaN(dt.getTime()))
            return { ok: false, error: "invalid_datetime", detail: "Could not parse datetime string" };
        // Validate timezones
        let fromFormatted = null;
        if (from_tz) {
            try {
                fromFormatted = new Intl.DateTimeFormat("en-US", { timeZone: from_tz, dateStyle: "full", timeStyle: "long" }).format(dt);
            }
            catch {
                return { ok: false, error: "invalid_from_tz", detail: `Unknown timezone: ${from_tz}` };
            }
        }
        if (!to_tz)
            return { ok: false, error: "missing_to_tz" };
        let toFormatted;
        try {
...
async function sentimentAnalysis(payload) {
    ...
    if (text.length > 50_000)
        return { ok: false, error: "text_too_long", max: 50_000 };
    const result = await claudeJson(`You are a sentiment analysis engine. Always respond with valid JSON only. No markdown, no explanation.`, `Analyze the sentiment of this text and return JSON with this exact structure:
{
...
async function summarize(payload) {
    ...
    if (text.length > 100_000)
        return { ok: false, error: "text_too_long", max: 100_000 };
    const styles = ["paragraph", "bullets", "tldr", "headline", "executive"];
    if (!styles.includes(style))
        return { ok: false, error: "unsupported_style", supported: styles };
    ...
    const result = await claudeJson(`You are a professional summarizer. Always respond with valid JSON only. No markdown backticks.`, `${styleInstructions[style]}
...
async function extractEntities(payload) {
    ...
    const requestedTypes = Array.isArray(types) && types.length ? types.filter((t) => allTypes.includes(t)) : allTypes;
    const result = await claudeJson(`You are a named entity recognition engine. Always respond with valid JSON only.`, `Extract named entities from the text. Return JSON where each key is an entity type and value is an array of found entities (strings, deduplicated).
...
async function languageDetect(payload) {
    ...
    if (!text || typeof text !== "string")
        return { ok: false, error: "missing_text" };
    const result = await claudeJson(`You are a language detection engine. Always respond with valid JSON only.`, `Detect the language of this text. Return JSON:
{
...
async function piiDetect(payload) {
    ...
    if (text.length > 50_000)
        return { ok: false, error: "text_too_long", max: 50_000 };
    const result = await claudeJson(`You are a PII detection engine. Always respond with valid JSON only.`, `Detect all PII (Personally Identifiable Information) in the text.
...
${text.slice(0, 10_000)}`);
    ...
    if (redact && result.data.findings?.length) {
        ...
        for (const f of findings) {
            if (f.value)
                rt = rt.split(f.value).join(replacement);
        }
        redacted_text = rt;
    }
    return {
...
async function rssParse(payload) {
    ...
    try {
        ...
        if (!channel)
            return { ok: false, error: "invalid_feed", detail: "Could not parse RSS/Atom structure" };
        ...
        const items = rawItems.slice(0, Math.min(Number(limit) || 20, 100)).map((item) => {
            const title = item.title?.__cdata || item.title || "";
            const link = isAtom
                ? (Array.isArray(item.link) ? item.link.find((l) => l["@_rel"] !== "self")?.["@_href"] : item.link?.["@_href"] || item.link)
                : item.link || "";
...
async function generateUuid(payload) {
    ...
    if (!supported.includes(type))
        return { ok: false, error: "unsupported_type", supported };
...
async function regexGenerate(payload) {
    ...
    const result = await claudeJson(`You are a regex generation expert. Always respond with valid JSON only. No markdown.`, `Generate a regular expression for this requirement: "${description}"
...
Make the regex as precise and practical as possible.`);
    ...
    try {
        ...
        if (Array.isArray(test_strings) && test_strings.length) {
            result.data.examples = test_strings.map((s) => ({
                input: s,
                matches: re.test(s),
...
async function diffText(payload) {
    ...
    try {
        ...
        if (format === "words") {
            ...
            changes.forEach((c) => {
                if (c.added)
                    stats.added += (c.value.match(/\S+/g) || []).length;
                else if (c.removed)
                    stats.removed += (c.value.match(/\S+/g) || []).length;
                else
                    stats.unchanged += (c.value.match(/\S+/g) || []).length;
            });
            return { ok: true, format: "words", changes: changes.map((c) => ({ type: c.added ? "+" : c.removed ? "-" : "=", value: c.value })), stats };
        }
        if (format === "chars") {
            ...
            changes.forEach((c) => {
                if (c.added)
                    stats.added += c.count || 0;
                else if (c.removed)
                    stats.removed += c.count || 0;
                else
                    stats.unchanged += c.count || 0;
            });
            return { ok: true, format: "chars", changes: changes.map((c) => ({ type: c.added ? "+" : c.removed ? "-" : "=", value: c.value })), stats };
        }
        // json format
        const changes = diffLines(String(original), String(modified));
        const json = changes.map((c) => ({ type: c.added ? "added" : c.removed ? "removed" : "unchanged", lines: c.value.split("\n").filter(Boolean) }));
        const added = json.filter((c) => c.type === "added").reduce((s, c) => s + c.lines.length, 0);
        const removed = json.filter((c) => c.type === "removed").reduce((s, c) => s + c.lines.length, 0);
...
async function webSearch(payload) {
    ...
    if (query.length > 500)
        return { ok: false, error: "query_too_long", max: 500 };
    const apiKey = process.env.TAVILY_API_KEY;
    if (!apiKey)
        return { ok: false, error: "search_not_configured", detail: "TAVILY_API_KEY not set" };
    try {
        const ctrl = new AbortController();
        const t = setTimeout(() => ctrl.abort(), 15_000);
...
async function whoisLookup(payload) {
    ...
    try {
        ...
        clearTimeout(t);
        if (!r.ok)
            return { ok: false, error: "lookup_failed", status: r.status, detail: "Domain not found or RDAP not available" };
        const data = (await r.json());
        // Parse RDAP response
        const events = {};
        ...
        const nameservers = (data.nameservers || []).map((ns) => (ns.ldhName || ns.unicodeName || "").toLowerCase()).filter(Boolean);
        const entities = (data.entities || []);
        const registrar = entities.find((e) => e.roles?.includes("registrar"));
        const registrarName = registrar?.vcardArray?.[1]?.find((f) => f[0] === "fn")?.[3] || registrar?.publicIds?.[0]?.identifier || "";
        const status = (data.status || []).map((s) => s.replace("client ", "").replace("server ", ""));
        return {
            ok: true,
            domain: cleaned,
...
async function searchWeb(payload) {
    ...
    if (process.env.TAVILY_API_KEY) {
        try {
            ...
            const data = await resp.json();
            if (!resp.ok)
                return { ok: false, error: "search_failed", provider: "tavily", status: resp.status, detail: data };
    ...
    if (process.env.SERPER_API_KEY) {
        try {
            ...
            const data = await resp.json();
            if (!resp.ok)
                return { ok: false, error: "search_failed", provider: "serper", status: resp.status, detail: data };
            const results = (data.organic || []).slice(0, n).map((r) => ({
    ...
    try {
        ...
        if (!resp.ok)
            return { ok: false, error: "search_failed", provider: "duckduckgo", status: resp.status };
...
async function extractPdf(payload) {
    ...
    try {
        ...
        if (!resp.ok)
            return { ok: false, error: "pdf_extract_failed", status: resp.status, detail: data };
...
async function imageGenerate(payload) {
    ...
    if (prompt.length > 4000)
        return { ok: false, error: "prompt_too_long", max: 4000 };
    ...
    if (openaiKey && (!model || model.startsWith("dall-e"))) {
        try {
            ...
            const data = (await resp.json());
            if (!resp.ok)
                return { ok: false, error: "image_generation_failed", detail: data?.error?.message || String(resp.status) };
...
async function htmlToMarkdown(payload) {
    ...
    if (!raw || typeof raw !== "string")
        return { ok: false, error: "missing_html_or_url" };
...
async function urlShorten(payload) {
    ...
    try {
        ...
        const data = (await r.json());
        if (data.errorcode)
            return { ok: false, error: "shorten_failed", detail: data.errormessage || `Error ${data.errorcode}` };
...
async function webhookSend(payload) {
    ...
    if (!allowedMethods.includes(m))
        return { ok: false, error: "unsupported_method", allowed: allowedMethods };
    try {
        await assertSafeUrl(url);
        const ctrl = new AbortController();
        const t = setTimeout(() => ctrl.abort(), TOOL_TIMEOUT_MS);
...
async function jsonpathQuery(payload) {
    ...
    if (typeof json === "string") {
        try {
            ...
            return { ok: false, error: "invalid_json", detail: e.message };
        }
    }
...
function query(node, tokens) {
    ...
    if (head === "..") {
        ...
        function descend(n) {
            ...
            if (Array.isArray(n))
                ...
                Object.values(n).forEach(descend);
        }
        descend(node);
        return results;
    }
    if (head === "*") {
        if (Array.isArray(node))
            return node.flatMap((v) => query(v, rest));
        if (node && typeof node === "object")
            return Object.values(node).flatMap((v) => query(v, rest));
        return [];
    }
    if (head.startsWith("[")) {
...
async function barcodeGenerate(payload) {
    ...
    if (!supported.includes(fmt))
        return { ok: false, error: "unsupported_format", supported };
...
function calcEanCheckDigit(digits) {
    let sum = 0;
    for (let i = 0; i < 12; i++)
        sum += Number(digits[i]) * (i % 2 === 0 ? 1 : 3);
    return String((10 - (sum % 10)) % 10);
}
function calcUpcCheckDigit(digits) {
    let sum = 0;
    for (let i = 0; i < 11; i++)
        sum += Number(digits[i]) * (i % 2 === 0 ? 3 : 1);
    return String((10 - (sum % 10)) % 10);
}
function generateCode128Svg(value, w, h, showText) {
    ...
    for (const b of bars) {
        ...
        if (b.fill === "black")
            rects += `<rect x="${x.toFixed(1)}" y="5" width="${bw.toFixed(1)}" height="${h - (showText ? 20 : 5)}" fill="black"/>\n`;
        x += bw;
    }
...
function generateCode39Svg(value, w, h, showText) {
...
function generateEan13Svg(digits, w, h, showText) {
    ...
    for (let i = 0; i < totalBits; i++) {
        if (bits[i] === "1")
            rects += `<rect x="${(10 + i * barW).toFixed(2)}" y="5" width="${barW.toFixed(2)}" height="${h - (showText ? 20 : 5)}" fill="black"/>\n`;
    }
    const textEl = showText ? `<text x="${w / 2}" y="${h - 3}" text-anchor="middle" font-family="monospace" font-size="12">${digits}</text>` : "";
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><rect width="${w}" height="${h}" fill="white"/>${rects}${textEl}</svg>`;
}
function generateUpcaSvg(digits, w, h, showText) {
