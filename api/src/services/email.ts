...
export async function sendWelcomeEmail(to: string, agentId: string, apiKey: string, creditsGranted: number, referralCode?: string): Promise<void> {
  ...
  const html = layout(subject, `
    ...
    <p>That's <strong>${creditsGranted.toLocaleString()} free credits</strong> to explore all 63 AI tools — a starter allowance is usable immediately, and the rest unlock when you verify your email.</p>
