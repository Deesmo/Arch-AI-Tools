...
export function calculateBadge(totalCalls: number): Badge {
  if (totalCalls >= BADGE_THRESHOLDS.diamond) return "diamond";
  if (totalCalls >= BADGE_THRESHOLDS.gold) return "gold";
  if (totalCalls >= BADGE_THRESHOLDS.silver) return "silver";
  if (totalCalls >= BADGE_THRESHOLDS.bronze) return "bronze";
  return "none";
}
...
export function calculateReputationScore(params: {
  ...
  if (totalCalls === 0) return 50; // neutral starting score
...
export async function updateAgentReputation(agentId: string): Promise<void> {
  try {
...
export async function recordAgentCall(agentId: string, success: boolean): Promise<void> {
  try {
    await prisma.agent.update({
      where: { id: agentId },
      data: success
...
// ─── Record Spend ────────────────────────────────────────────────────────────
export async function recordAgentSpend(agentId: string, amountUsdc: number): Promise<void> {
  try {
    await prisma.agent.update({
      where: { id: agentId },
