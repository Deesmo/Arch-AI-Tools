...
export interface AgentFingerprint {
  ...
  callerName: string;  // e.g. "claude-desktop", "cursor", "openai-sdk", "python"
  callerVersion?: string;
}

const AI_AGENT_PATTERNS: Array<{ pattern: RegExp; name: string }> = [
  // MCP clients
...
export function fingerprintCaller(userAgent?: string): AgentFingerprint {
  if (!userAgent) {
  ...
  for (const { pattern, name } of AI_AGENT_PATTERNS) {
    if (pattern.test(userAgent)) {
      ...
      let callerType: AgentFingerprint["callerType"];
