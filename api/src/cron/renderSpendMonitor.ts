...
export async function runRenderSpendMonitor(): Promise<void> {
  if (!RENDER_API_KEY) {
  ...
  try {
    ...
    if (proServices.length > 8) {
      await sendAdminAlert(
        "⚠️ Render Spend Alert — Arch Tools",
        `${proServices.length} pro/standard services running. Review for unused services.\n\nhttps://dashboard.render.com/billing`
      );
    }
  } catch (err: any) {
