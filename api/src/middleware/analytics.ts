...
const hourlyVolumes: Map<string, number[]> = new Map();
// Active alerts
const activeAlerts: UsageAlert[] = [];
const MAX_ALERTS = 500;
// Agents already flagged (avoid spam)
const flaggedAgents: Map<string, number> = new Map(); // agentId → last flagged timestamp
...
function trackRateLimitHit(agentId: string): void {
  const now = Date.now();
...
function trackHourlyVolume(agentId: string): void {
  const now = Date.now();
  const oneHourAgo = now - 60 * 60 * 1000;
  ...
  const agentMetrics24h = recentMetrics.filter(
    m => m.agentId === agentId && m.timestamp > twentyFourHoursAgo
  );
 ...
 */
export function checkCreditThreshold(agentId: string, creditsRemaining: number, totalCredits: number): void {
...
function addAlert(params: Omit<UsageAlert, "id" | "timestamp" | "acknowledged">): void {
  const now = Date.now();
  const lastFlagged = flaggedAgents.get(`${params.agentId}:${params.type}`);

  // Cooldown: don't spam the same alert type for the same agent
  if (lastFlagged && now - lastFlagged < FLAG_COOLDOWN_MS) return;
...
export function getActiveAlerts(limit: number = 50): UsageAlert[] {
  return activeAlerts
...
export function getAllAlerts(limit: number = 100): UsageAlert[] {
...
export function getRateLimitViolators(): Array<{ agentId: string; hits: number }> {
  const now = Date.now();
  const oneHourAgo = now - 60 * 60 * 1000;
  const result: Array<{ agentId: string; hits: number }> = [];
