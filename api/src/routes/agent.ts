...
agentRouter.post(
  "/v1/agent/register",
  rateLimitPolicyMiddleware(5, 60 * 60 * 1000),
  registerLimiter,
  async (req, res) => {
 ...
 */
agentRouter.post("/v1/agent/keys", requireApiKey, async (req: any, res) => {
 ...
 */
agentRouter.delete("/v1/agent/keys/:prefix", requireApiKey, async (req: any, res) => {
  const agentId = req.agentId as string;
...
function scoreTools(task: string, tools: any[]) {
  ...
  return tools
    .map((t) => {
      ...
      if (/(news|latest|web|site|page|scrape|extract|browser)/.test(q) && /(web|files|ai)/.test(t.category || "")) score += 1;
      return { t, score };
    })
    .filter((x) => x.score > 0)
...
agentRouter.post("/v1/agent/execute", requireApiKey, async (req: any, res) => {
  const agentId = req.agentId as string;
  const task = String(req.body?.task || "").trim();
  if (!task) return fail(req, res, 400, "invalid_request", "Missing task");
  ...
  const steps: Array<{ tool: string; input: any; credits: number }> = [];

  const has = (name: string) => toolRows.some((t) => t.name === name);
  const toolCost = (name: string) => toolRows.find((t) => t.name === name)?.credits || 0;
  ...
  if (wantsWeb && has("extract-page") && /(http:\/\/|https:\/\/)/i.test(task)) {
    ...
    if (match) steps.push({ tool: "extract-page", input: { url: match[0] }, credits: toolCost("extract-page") });
  }
  ...
  for (let i = 0; i < bounded.length; i++) {
    ...
    const resolved = JSON.parse(
      JSON.stringify(step.input ?? {}, (_k, v) => {
    ...
    try {
      ...
      try {
        await (prisma as any).apiRequest?.create?.({
          data: {
            agentId,
            tool: step.tool,
            status: "ok",
...
agentRouter.post("/v1/agent/:agentId/reverse", requireAdminKey, async (req, res) => {
  ...
  if (!Number.isFinite(credits) || credits <= 0) return res.status(400).json({ error: "invalid_credits" });
 ...
 */
agentRouter.get("/v1/agent/dashboard", requireApiKey, async (req: any, res) => {
  const agentId = req.agentId as string;
  const now = new Date();
