...
async function ensureMonthlyFreeCredits(agentId: string) {
...
authRouter.post("/v1/auth/signup", signupLimiter, async (req, res) => {
  ...
  const ip = String((req.headers["x-forwarded-for"] || req.ip || "") as any).slice(0, 200);
...
authRouter.get("/v1/auth/verify", async (req, res) => {
  ...
  if (!record) {
    ...
    return res.send(
      `<!DOCTYPE html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Invalid link</title></head><body style="font-family:system-ui;padding:24px;">` +
 ...
 * Requires API key.
 */
authRouter.get("/v1/whoami", requireApiKey, async (req: any, res) => {
