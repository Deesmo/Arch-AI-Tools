...
router.get("/listings", requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
...
router.post("/listings/:id/approve", requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (service) {
      if (listing.tier === "featured") service.featured = true;
      if (listing.tier === "verified") service.verified = true;
      saveDirectory(data);
    }
...
router.post("/listings/:id/reject", requireAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
