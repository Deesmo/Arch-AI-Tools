...
dashboardRouter.get("/v1/dashboard/usage", requireApiKey, async (req: any, res) => {
  ...
  try {
    series = await prisma.$queryRawUnsafe(
      `SELECT date_trunc('day', "createdAt")::date as day, count(*)::int as calls, sum("creditsUsed")::int as credits
       FROM api_requests
    ...
    // ledger fallback
    series = await prisma.$queryRawUnsafe(
      `SELECT date_trunc('day', "createdAt")::date as day, count(*)::int as calls, sum("credits")::int as credits
       FROM "LedgerEntry"
...
dashboardRouter.get("/v1/dashboard/requests", requireApiKey, async (req: any, res) => {
  const agentId = req.agentId as string;
  const limit = Math.min(Math.max(Number(req.query?.limit) || 50, 1), 200);

  try {
    const rows = await prisma.$queryRawUnsafe(
      `SELECT "createdAt" as ts, tool, status, "latencyMs" as latency_ms, "creditsUsed" as credits_used, "requestId" as request_id
       FROM api_requests
       WHERE "agentId" = $1
    ...
    return res.json({
      ...
      requests: rows.map((r) => ({
        ...
        tool: r.toolName,
        status: "ok",
        latency_ms: (r.meta as any)?.latency_ms ?? null,
...
dashboardRouter.get("/v1/dashboard/api-keys", requireApiKey, async (req: any, res) => {
  const agentId = req.agentId as string;
