  ...
  res.json({
    ...
    flow: {
      ...
      step_2_payment_required: {
        description: "Server responds with 402 + x402 v2 payment instructions (the base64 PAYMENT-REQUIRED header carries the same PaymentRequired JSON as the body)",
        status: 402,
        headers: {
          "PAYMENT-REQUIRED": "<base64-encoded PaymentRequired — same JSON as the body>",
          "Content-Type": "application/json",
        },
        body: {
          x402Version: 2,
      ...
      step_3_payment: {
        description: "Agent signs USDC transfer and retries with the PAYMENT-SIGNATURE header (base64 v2 PaymentPayload; legacy v1 X-Payment payloads are still accepted)",
        method: "POST",
        url: resource,
        headers: {
          "Content-Type": "application/json",
          "PAYMENT-SIGNATURE": "eyJ4NDAyVmVyc2lvbiI6MiwiYWNjZXB0ZWQiOnsic2NoZW1lIjoiZXhhY3QiLCJuZXR3b3JrIjoiZWlwMTU1Ojg0NTMifSwicGF5bG9hZCI6eyJzaWduYXR1cmUiOiIweC4uLiJ9fQ==",
        },
...
function getSampleParams(tool: string): Record<string, unknown> {
  const samples: Record<string, Record<string, unknown>> = {
    ...
    "summarize": { text: "Arch Tools provides 63 production-ready API tools for developers and AI agents.", style: "tldr" },
...
function getDemoResult(tool: string): unknown {
  const results: Record<string, unknown> = {
    ...
    "ai-generate": { text: "Arch Tools gives AI agents 63 production-ready APIs with built-in x402 USDC payments — no API key required." },
