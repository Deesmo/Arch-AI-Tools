...
router.use(requireAuth, requireAccountAuth);
...
router.post("/test", async (req: AuthedRequest, res: Response): Promise<void> => {
  const agent = req.agent;
  if (!agent) { res.status(401).json({ ok: false, error: "unauthorized", request_id: reqId() }); return; }

  const { webhook_id } = req.body as { webhook_id?: string };
