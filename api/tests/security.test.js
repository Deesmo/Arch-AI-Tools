...
async function main() {
  ...
  const billingRouteSrc = fs.readFileSync(
    path.join(__dirname, "..", "dist", "routes", "billing.js"), "utf-8");
  test("Stripe cancellation persistence failure returns retryable 500", () => {
    ...
    assert.ok(!/tier:\s*["']free["'][\s\S]{0,180}\.catch\(\(\)\s*=>\s*\{\}\)/.test(billingRouteSrc),
      "billing route must not swallow tier downgrade failures");
  });

  const agentRouteSrc = fs.readFileSync(
    path.join(__dirname, "..", "dist", "routes", "agent.js"), "utf-8");
  test("API-key revocation persistence failure does not report success", () => {
    ...
    assert.ok(!/apiKeyPrefix:\s*null[\s\S]{0,180}\.catch\(\(\)\s*=>\s*\{\}\)/.test(agentRouteSrc),
      "agent route must not swallow API-key revocation failures");
  });

  const toolsRouteSrc = fs.readFileSync(
    path.join(__dirname, "..", "dist", "routes", "tools", "index.js"), "utf-8");
