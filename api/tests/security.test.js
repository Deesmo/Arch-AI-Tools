...
async function main() {
  ...
  // ── FaaS: provider network allowlist is enforced before settle ────────────
  const facilitator = await import(path.join(__dirname, "..", "dist", "routes", "facilitator.js"));
  const { validatePaymentDetailsForProvider } = facilitator;
